...
exports.PlannerViewProvider = void 0;
...
class PlannerViewProvider {
    ...
    resolveWebviewView(webviewView, _context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.type) {
                case 'analyze':
                    await this._handleAnalyze(msg.requirement ?? '', msg.modelIds ?? []);
                    break;
                case 'cancel':
                    this._cts?.cancel();
                    break;
                case 'copy':
                    ...
                    break;
                case 'openChat':
                    await this._openChat(msg.text ?? '');
                    break;
                case 'requestModels':
                    await this._fetchAndSendModels();
                    break;
                case 'previewSkills':
                    await this._previewSkills(msg.requirement ?? '');
                    break;
                ...
                case 'requestBrowse':
                    ...
                    break;
                case 'analyzeChat':
                    await this._handleAnalyzeChat(msg.conversation ?? '', msg.modelIds ?? []);
                    break;
                case 'generatePlanFromTasks':
                    await this._handleGeneratePlan(msg.text ?? '', msg.modelIds ?? []);
                    break;
            }
        });
    ...
    // ─── 需求分析 ─────────────────────────
    async _handleAnalyze(requirement, modelIds) {
        if (!requirement.trim()) {
        ...
        try {
            ...
            const plan = await (0, LmAnalyzer_1.runWorkflow)(requirement, skills, modelIds, (n) => this._post({ type: 'nodeStart', skillName: n }), (n, a) => this._post({ type: 'nodeDone', skillName: n, analysis: a }), (n, e) => this._post({ type: 'nodeError', skillName: n, error: e }), (m) => this._post({ type: 'status', text: m }), token);
    ...
    // ─── 对话分析 ─────────────────────────
    async _handleAnalyzeChat(conversation, modelIds) {
        ...
        this._isAnalyzing = true;
        try {
            const result = await (0, LmAnalyzer_1.analyzeConversation)(conversation, modelIds, (m) => this._post({ type: 'chatStatus', text: m }), token);
    ...
    async _handleGeneratePlan(tasksJson, modelIds) {
        try {
...
exports.PlannerViewProvider = PlannerViewProvider;
// ─── Webview HTML ─────────────────────────────────
...
//# sourceMappingURL=_old_PlannerViewProvider.js.map