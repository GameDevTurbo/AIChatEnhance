...
function getMultiplier(family, name, table) {
    if (!table) {
        table = getMultiplierTable();
    }
    // 1) 精确匹配 family
...
async function runModelParallelAnalysis(requirement, _skillContext, // 传递给 generatePlan 使用，分析阶段不使用
