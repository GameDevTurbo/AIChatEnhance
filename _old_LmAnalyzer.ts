...
function pickModels(
    ...
    let chosen: vscode.LanguageModelChat[] = modelIds.length > 0
        ? modelIds.map(id => allModels.find(m => m.id === id)).filter((m): m is vscode.LanguageModelChat => !!m)
        : [];
    if (!chosen.length) {
...
export async function runWorkflow(
    ...
    modelIds: string[],
    onNodeStart: (skillName: string) => void,
