...
export class PlannerPanel {
    public static readonly viewType = 'taskPlannerPanel';
    private static _instance: PlannerPanel | undefined;
    ...
    public static createOrShow(context: vscode.ExtensionContext): void {
        ...
        const panel = vscode.window.createWebviewPanel(
            PlannerPanel.viewType, 'Task Planner',
    ...
    private constructor(panel: vscode.WebviewPanel, context: vscode.ExtensionContext) {
        ...
        const saved = context.globalState.get<SavedSettings>(STATE_KEY_SETTINGS);
        const history = context.globalState.get<SessionRecord[]>(STATE_KEY_HISTORY, []);
        setTimeout(() => {
    ...
    private _saveSettings(s: Partial<SavedSettings>): void {
        const current = this._context.globalState.get<SavedSettings>(STATE_KEY_SETTINGS, {} as SavedSettings);
        this._context.globalState.update(STATE_KEY_SETTINGS, { ...current, ...s });
    }

    private _appendHistory(requirement: string, plan: string): void {
        const history = this._context.globalState.get<SessionRecord[]>(STATE_KEY_HISTORY, []);
        history.unshift({
    ...
    private _handleMessage(msg: WebviewMessage): void {
        switch (msg.type) {
            ...
            case 'requestAllSkills':
                this._post({ type: 'allSkills', skills: getAllSkillNames() });
                break;
            case 'attachImages':
            ...
            case 'loadHistorySession': {
                ...
                if (session) { this._post({ type: 'loadedSession', plan: session.plan }); }
                break;
            }
    ...
    private async _handleAttachImages(): Promise<void> {
    ...
    private async _handleAnalyze(requirement: string, mc: ModelConfig): Promise<void> {
        if (!requirement.trim()) {
        ...
        try {
            ...
            if (wsContext) { skillParts.unshift(wsContext); }
            const skillContext = skillParts.join('\n\n');
            this._lastSkillContext = skillContext;

            this._post({ type: 'modelAnalysisStart' });
            const results = await runModelParallelAnalysis(
                requirement, skillContext, mc,
                (id, name) => this._post({ type: 'modelStart', modelId: id, modelName: name }),
                (id, analysis) => this._post({ type: 'modelDone', modelId: id, analysis }),
                (id, error) => this._post({ type: 'modelError', modelId: id, error }),
    ...
    private async _handleAnalyzeChat(conversation: string, mc: ModelConfig): Promise<void> {
    ...
    private async _handleRegeneratePlan(mc: ModelConfig): Promise<void> {
    ...
    private async _handleGeneratePlan(mc: ModelConfig): Promise<void> {
        if (!this._lastModelResults.length) {
        ...
        try {
            ...
            const merged = await generatePlan(
                this._lastRequirement, this._lastModelResults, mc,
                (m) => this._post({ type: 'status', text: m }),
                token,
                this._attachedImages.length > 0 ? this._attachedImages : undefined,
    ...
    private async _handleExecuteWithContext(
        planText: string, attachSkills: boolean, attachImages: boolean, attachAnalysis: boolean
    ): Promise<void> {
        const parts: string[] = [];
    ...
    private _estimateTokens(attachSkills: boolean, attachImages: boolean, attachAnalysis: boolean): number {
        let chars = 0;
        if (attachSkills && this._lastSkillContext) { chars += this._lastSkillContext.length; }
        if (attachAnalysis) {
            for (const r of this._lastModelResults) {
                if (r.analysis) { chars += r.analysis.length; }
            }
        }
        if (attachImages) { chars += this._attachedImages.length * 3000; }
        return Math.round(chars / 4);
    }

    private _sendTokenEstimate(attachSkills: boolean, attachImages: boolean, attachAnalysis: boolean): void {
    ...
    private async _sendBrowseData(): Promise<void> {
        const skills = await getSkillDescriptions();
        const templates = await scanPromptTemplates();
        this._post({
            type: 'browseData',
    ...
    private async _fetchAndSendModels(): Promise<void> {
        try {
            ...
            const hidden = cfg.get<string[]>('hiddenModels', []);
            const models = await getAvailableModels(hidden);
