...
import { runWorkflow, getAvailableModels, analyzeConversation, ModelConfig } from './LmAnalyzer';
...
export class PlannerPanel {
    ...
    private _mc(msg: { primaryModelId?: string; secondaryModelIds?: string[] }): ModelConfig {
        return { primaryId: msg.primaryModelId ?? '', secondaryIds: msg.secondaryModelIds ?? [] };
    }

    private async _handleMessage(msg: Record<string, unknown>): Promise<void> {
        const mc = this._mc(msg as { primaryModelId?: string; secondaryModelIds?: string[] });
        switch (msg.type) {
            case 'analyze':          await this._handleAnalyze(String(msg.requirement ?? ''), mc); break;
            case 'cancel':           this._cts?.cancel(); break;
            case 'copy':
                await vscode.env.clipboard.writeText(String(msg.text ?? ''));
                vscode.window.showInformationMessage('已复制到剪贴板');
                break;
            case 'openChat':         await this._openChat(String(msg.text ?? '')); break;
            case 'requestModels':    await this._fetchAndSendModels(); break;
            case 'previewSkills':    await this._previewSkills(String(msg.requirement ?? '')); break;
            ...
            case 'analyzeChat':      await this._handleAnalyzeChat(String(msg.conversation ?? ''), mc); break;
            case 'generatePlanFromTasks': await this._handleGeneratePlan(String(msg.text ?? ''), mc); break;
        }
    }
    ...
    private async _handleAnalyze(requirement: string, mc: ModelConfig): Promise<void> {
        ...
        try {
            ...
            const plan = await runWorkflow(
                requirement, skills, mc,
                (n, m) => this._post({ type: 'nodeStart', skillName: n, modelName: m }),
    ...
    private async _handleGeneratePlan(tasksJson: string, mc: ModelConfig): Promise<void> {
        try {
