...
export async function runWorkflow(
    ...
    onProgress(`主模型: ${primary.name} | 参与: ${modelNames} | 并行处理 ${skills.length} 个节点`);
    ...
    onProgress(`所有节点完成，主模型 (${primary.name}) 正在汇总执行计划...`);

    const summaryMessages = [
        vscode.LanguageModelChatMessage.User(
            `你是 PackJam Unity 项目的 AI 规划助手。\n用户需求：${requirement}\n\n` +
            `各专项节点分析结果：\n\n` +
...
\`\`\``
    ...
    const summaryResponse = await primary.sendRequest(summaryMessages, {}, token);
