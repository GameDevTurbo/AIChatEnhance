...
/** 合并默认 + 用户配置的倍率表 */
function getMultiplierTable(): Record<string, number> {
    const overrides = vscode.workspace.getConfiguration('aichatEnhance')
        .get<Record<string, number>>('modelMultipliers', {});
    return { ...DEFAULT_MULTIPLIERS, ...overrides };
}

// 片段级 fallback：当精确匹配失败时用关键词推断
const MULTIPLIER_KEYWORDS: Array<[string, number]> = [
    ['haiku',  0.33],
    ['flash',  0.33],
    ['mini',   0.33],
    ['sonnet', 1],
...
function getMultiplier(family: string, name: string): { value: number; source: 'table' | 'keyword' } | undefined {
    ...
    for (const [k, v] of sorted) {
        if (nameNorm.includes(k)) { return { value: v, source: 'table' }; }
    }
    // 3) 关键词 fallback
    const combined = `${key} ${nameNorm}`;
    for (const [kw, v] of MULTIPLIER_KEYWORDS) {
        if (combined.includes(kw)) { return { value: v, source: 'keyword' }; }
    }
    return undefined;
}
...
export async function getAvailableModels(hiddenPatterns?: string[]): Promise<ModelInfo[]> {
    ...
    let result = models.map(m => {
        ...
        return {
            ...
            multiplierSource: apiMult !== undefined ? 'api' as const
...
function resolveParticipants(
    ...
    config: ModelConfig
): { primary: vscode.LanguageModelChat; all: vscode.LanguageModelChat[] } {
    let primary: vscode.LanguageModelChat | undefined;
    if (config.primaryId) {
    ...
    const secondaries = config.secondaryIds
        ...
        .filter((m): m is vscode.LanguageModelChat => !!m && m.id !== primary!.id);
...
export async function runModelParallelAnalysis(
    ...
    const promises = assignments.map(async ({ perspective, model }): Promise<ModelAnalysisResult> => {
...
export async function analyzeConversation(
    ...
    const mergeMessages = [
        vscode.LanguageModelChatMessage.User(
            `你是任务规划专家。三个视角（产品经理、架构师、项目经理）分别从一段对话中提取了任务：\n\n` +
            `产品经理：\n${rawResults[0]}\n\n` +
            `架构师：\n${rawResults[1]}\n\n` +
            `项目经理：\n${rawResults[2]}\n\n` +
            `请合并去重，输出最终任务列表。格式：\n` +
    ...
    try {
        ...
        const parsed = JSON.parse(jsonMatch[0]) as { summary?: string; tasks?: ExtractedTask[] };
        return {
